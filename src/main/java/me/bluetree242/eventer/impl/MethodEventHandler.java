package me.bluetree242.eventer.impl;
...
public class MethodEventHandler implements EventHandler {
    ...
    @Getter private final Class event;
    @Getter private final DiscordListener listener;
    ...
    public MethodEventHandler(Method method, DiscordListener listener) {
        ...
        if (method.getParameterCount() != 1) throw new BadListenerException("@SubscribeEvent on a method with more than/less than 1 parameter");
        if (!Eventer.events.contains(method.getParameterTypes()[0])) throw new IllegalStateException("Method " + method.toGenericString() + " first param is not an event");
